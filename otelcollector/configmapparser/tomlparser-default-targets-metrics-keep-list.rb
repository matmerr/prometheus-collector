...
@retinabasicRegex_minimal = "retina.*"
...
@retinabasicRegex_minimal_mac = "retina.*"
...
def populateSettingValuesFromConfigMap(parsedConfig)
  begin
    ...
    retinabasicRegex = parsedConfig[:retinabasic]
    if !retinabasicRegex.nil? && retinabasicRegex.kind_of?(String)
      if !retinabasicRegex.empty?
        if isValidRegex(retinabasicRegex) == true
          @retinabasicRegex = retinabasicRegex
          ConfigParseErrorLogger.log(LOGGING_PREFIX, "Using configmap metrics keep list regex for retinabasic")
        else
          ConfigParseErrorLogger.logError(LOGGING_PREFIX, "Invalid keep list regex for retinabasic")
        end
      end
    else
      ConfigParseErrorLogger.logError(LOGGING_PREFIX, "retinabasicRegex either not specified or not of type string")
    end
  ...
  begin
    if @minimalIngestionProfile == "true"
      if @isMacMode == true
        ...
        @retinabasicRegex = @retinabasicRegex + "|" + @retinabasicRegex_minimal_mac
        ...
        @retinabasicRegex = @retinabasicRegex + "|" + @retinabasicRegex_minimal
